...
module.exports = (io, socket, rooms) => {
  ...
  socket.on("reconnect_teacher", ({ teacherId }) => {
  console.log("teacher reconnect:", teacherId);

  // หา room ที่ teacher นี้เคยอยู่
  for (const joinCode in rooms) {
    const room = rooms[joinCode];

    // ถ้าห้องนี้เคยมี teacher
    if (!room.teacher) {
      room.teacher = { socketId: socket.id, teacherId };
      socket.join(joinCode);

      console.log("teacher restored to room:", joinCode);
    }
  }
});
};