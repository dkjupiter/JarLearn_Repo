...
module.exports = (socket) => {
      ...
      const studentRes = await db.query(`
        WITH student_question AS (
          ...
          JOIN "QuestionOptions" o
            ON o."Option_ID" = qa."Choice_ID"

          JOIN student_question sq
            ON sq."Student_ID" = ap."Student_ID"
  ...
  socket.on("get_class_report", async ({ classId }) => {
    ...
    `, [classId]);
      ...
      SELECT COUNT(DISTINCT asn."ActivitySession_ID") AS total_quiz
      FROM "ActivitySessions" asn
  ...
  socket.on("export_class_report_csv", async ({ classId }) => {
    ...
    `, [classId]);
      ...
      const studentAnswerRes = await db.query(`
        WITH student_question AS (
        ...
        ORDER BY "ActivitySession_ID","Question_ID"
        `, [classId]);
    ...
    `, [classId]);
      ...
      socket.emit("export_class_report_csv_data", {
        ...
        studentAnswers: studentAnswerRes.rows,
